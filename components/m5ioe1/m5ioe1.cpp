...
void M5IOE1Component::digital_write_hw(uint8_t pin, bool value) {
  ...
  // Per-pin read-modify-write on GPIO_O_L/GPIO_O_H (M5Unified M5IOE1_Class::digitalWrite).
  // Avoids rewriting all 16 output bits from output_mask_, which can stomp unrelated rails.
  const uint8_t reg = (pin < 8) ? REG_GPIO_O_L : REG_GPIO_O_H;
  const uint8_t bit = static_cast<uint8_t>(1 << (pin & 7));

  uint8_t reg_val = 0;
  if (!this->read_byte(reg, &reg_val)) {
  ...
  if (value) {
    reg_val |= bit;
    this->output_mask_ |= static_cast<uint16_t>(1 << pin);
  } else {
    reg_val &= static_cast<uint8_t>(~bit);
    this->output_mask_ &= static_cast<uint16_t>(~(1 << pin));
  }

  if (!this->write_byte(reg, reg_val)) {
    this->status_set_warning(LOG_STR("Failed to write GPIO output register"));
    return;
  }

#if ESPHOME_LOG_LEVEL >= ESPHOME_LOG_LEVEL_VERBOSE
  ESP_LOGV(TAG,
           "GPIO_OUT RMW pin=%u value=%d reg=0x%02X -> 0x%02X mask=0b" BYTE_TO_BINARY_PATTERN " " BYTE_TO_BINARY_PATTERN,
           pin, value, reg, reg_val, BYTE_TO_BINARY(this->output_mask_ >> 8),
           BYTE_TO_BINARY(this->output_mask_ & 0x00FF));
#endif
  this->status_clear_warning();
}
