...
bool St25r3916NfcA::read_fifo_(uint8_t *data, uint16_t capacity, uint16_t &actual) {
  ...
  const uint8_t opcode = OP_READ_FIFO;
  ESP_LOGD("papermono_nfc", "READ_FIFO opcode=0x%02X length=%u", opcode, actual);
  return device_->write_read(&opcode, 1, data, actual) == i2c::ERROR_OK;
}
...
bool St25r3916NfcA::anticollision_(uint8_t level, uint8_t result[5]) {
  // M5Stack's validated flow retries the complete anticollision exchange once
  // with no_crc_rx enabled if the normal receive framing does not complete.
  for (uint8_t attempt = 0; attempt < 2; ++attempt) {
    const bool no_crc_rx = attempt != 0;
    const uint8_t iso = 0x01;
    const uint8_t aux = no_crc_rx ? AUX_NO_CRC_RX : 0x00;
    ...
    ESP_LOGI("papermono_nfc", "CL%u anticollision attempt=%u no_crc_rx=%u TX=%02X %02X NVB=%02X",
             level, static_cast<unsigned>(attempt), no_crc_rx ? 1U : 0U, frame[0], frame[1], frame[1]);

    bool succeeded = false;
    for (uint32_t count = 0; count < 32; ++count) {
      if (!clear_interrupts_() || !command_(CMD_CLEAR_FIFO) ||
          !write_fifo_(frame, static_cast<uint16_t>(bytes + (bits != 0))) ||
          !write_tx_length_(bytes, bits) || !command_(CMD_TX_NOCRC)) {
        ESP_LOGW("papermono_nfc", "CL%u anticollision attempt=%u TX setup failed", level,
                 static_cast<unsigned>(attempt));
        break;
      }
      uint8_t irq[4]{};
      const bool interrupt = wait_for_interrupt_(IRQ_RXE | IRQ_COL, TIMER_NRE, 8, irq);
      ESP_LOGI("papermono_nfc", "CL%u anticollision attempt=%u IRQ=%02X %02X %02X %02X wait=%s",
               level, static_cast<unsigned>(attempt), irq[0], irq[1], irq[2], irq[3],
               interrupt ? "ok" : "timeout");
      if (!interrupt) break;
      const bool collision = (irq[0] & IRQ_COL) != 0;
      if (!collision && (irq[0] & IRQ_RXE) == 0) break;
      uint16_t actual{};
      if (!read_fifo_(result + offset, static_cast<uint16_t>(5 - offset), actual) || actual == 0) {
        ESP_LOGW("papermono_nfc", "CL%u anticollision attempt=%u FIFO empty/invalid", level,
                 static_cast<unsigned>(attempt));
        break;
      }
      ESP_LOGI("papermono_nfc", "CL%u anticollision attempt=%u FIFO length=%u collision=%s",
               level, static_cast<unsigned>(attempt), actual, collision ? "yes" : "no");
      if (collision) {
        uint8_t display{};
        if (!read_reg_(0x20, &display, 1)) break;
        ESP_LOGI("papermono_nfc", "CL%u collision_display=0x%02X", level, display);
    ...
    if (succeeded) {
      const bool bcc_ok = (result[0] ^ result[1] ^ result[2] ^ result[3]) == result[4];
      ESP_LOGI("papermono_nfc", "CL%u UID+BCC=%02X %02X %02X %02X %02X BCC=%s",
               level, result[0], result[1], result[2], result[3], result[4], bcc_ok ? "valid" : "invalid");
      if (bcc_ok) return true;
      ESP_LOGW("papermono_nfc", "CL%u anticollision attempt=%u failed: invalid BCC", level,
               static_cast<unsigned>(attempt));
    }
    if (attempt == 0) {
      ESP_LOGW("papermono_nfc", "CL%u anticollision retry: no_crc_rx=1", level);
    }
  }
  return false;
}
